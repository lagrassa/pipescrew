...
def test_collision_one_wall():
    ...
    assert(len(far_belief.find_collisions()) == 0)
    collide_belief = Belief(mu=(0.2, 0.09), cov = 0.1, walls = [wall])
    import ipdb; ipdb.set_trace()
    assert(len(collide_belief.find_collisions()) > 0)
    print("test passed")
...
def test_wall_close():
    ...
    import ipdb; ipdb.set_trace()
    assert( len(wall_1.in_collision(far_belief)) == 0)
    close_belief = Belief(mu=(0.2, 0.09), cov = 0.1, walls = [wall_1])
    assert(len(wall_1.in_collision(close_belief)) > 0)
