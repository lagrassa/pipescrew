...
def get_joint_limits(body, joint):
    ...
    joint_info = get_joint_info(body, joint)
    if joint_info.jointType != 4:
        assert(joint_info.jointLowerLimit <= joint_info.jointUpperLimit)
    else:
...
def get_collision_fn(body, joints, obstacles, attachments, self_collisions, disabled_collisions,
    ...
    def collision_fn(q):
        if not all_between(lower_limits, q, upper_limits):
            ...
            return True
        old_q = get_joint_positions(body, joints)
        old_poses = [get_pose(attachment.child) for attachment in attachments]
...
def inverse_kinematics_helper(robot, link, target_pose, movable_joints=None, null_space=None):
    ...
    if null_space is not None:
        ...
        upper = get_max_limits(robot, movable_joints) 
        lower = get_min_limits(robot, movable_joints) 
        kinematic_conf = p.calculateInverseKinematics(robot, link, target_point, None, upperLimits=upper, lowerLimits=lower,physicsClientId=CLIENT, residualThreshold=1e-3)
    else:
        if movable_joints is None:
            movable_joints = get_movable_joints(robot)
        upper = get_max_limits(robot, movable_joints) 
        lower = get_min_limits(robot, movable_joints) 
        kinematic_conf = p.calculateInverseKinematics(robot, link, target_point, target_quat, upperLimits=upper, lowerLimits=lower,physicsClientId=CLIENT, residualThreshold=1e-3)
...
def inverse_kinematics(robot, link, target_pose, movable_joints = None, max_iterations=200, null_space = None, custom_limits={}, **kwargs):
    if movable_joints is None:
        movable_joints = get_movable_joints(robot)
    for iterations in range(max_iterations):
        # TODO: stop is no progress
        # TODO: stop if collision or invalid joint limits
        kinematic_conf = inverse_kinematics_helper(robot, link, target_pose, movable_joints = movable_joints, null_space=null_space)
